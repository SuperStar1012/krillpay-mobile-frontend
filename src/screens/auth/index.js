...
import { initWithoutToken, initWithToken, verifyToken, getPublicCompany, getCompanyAppConfig } from "utility/rehive";

import { companiesSelector, currentSessionsSelector, authLogoutSelector } from "./redux/selectors";

import client from "config/client";

import { Spinner, MultiFactorAuthentication, Text, View } from "components";
...
export default function AuthScreen(props) {
	...
	const { items: currentSessions, userID } = useSelector(currentSessionsSelector);
	let company = null;
	const dispatch = useDispatch();
	const { showToast } = useToast();

	if (tempCompanyID && tempCompany) {
		companyID = client.company ? client.company : get(companies, "tempCompanyID");
		company = tempCompany;
	}
	...
	return (
		<LanguageProvider localLocales={locales} configLocales={authConfig?.locales ?? {}}>
			<View screen bC={"authScreen"}>
				<CompanyStatusBanner company={company} />
				{current.matches(AUTH_INIT) ? (
					<Spinner type="auth" size="large" color="authScreenContrast" backgroundColor="authScreen" />
					...
					<CurrentSessionsDrawer
						...
						{current.matches(LANDING) ? (
							<LandingPage {...pageProps} onLogin={() => send(LOGIN)} onGroup={onGroup} onRegister={() => send(REGISTER)} />
						) : (
			...
			<Disclaimer company={company} hideOverride={loading || current.matches(AUTH_SUCCESS)} />
			<ModalBasic visible={loading} animationType={"none"} backdropOpacity={0} presentationStyle={"overFullScreen"} transparent={true}>
				<View f={1} jC={"center"} aI={"center"}>
					<Spinner type="auth" size="large" color="authScreenContrast" backgroundColor="authScreen" />
				</View>
			</ModalBasic>
