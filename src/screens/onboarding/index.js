...
export default function Onboarding(props) {
  ...
  const [state, setState] = useState('form'); //set to form to enable onboarding form
  useEffect(() => {
    // if (sections?.length && completedSections < sections?.length) {
    if (sections?.length && completedSections < 0) {
      setCurrentSection(completedSections); // uncomment to enable onboarding form
      setState('form'); //uncomment to enable onboarding form
    }
  ...
  let activeSection = sections.length && sections[2];
