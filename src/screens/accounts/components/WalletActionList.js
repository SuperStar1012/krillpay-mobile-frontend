...
const actionsConfig = [
  ...
  },
  {
    ...
    condition: ({
      ...
      const hide = hideAction('exchange', { actionsConfig, currency, profile });
      if (hide) {
        return false;
      }
      if (
  ...
  function onButtonPress(action) {
    switch (action) {
      ...
      case 'withdraw':
        ...
        break;
      case 'deposit':
        navigation.navigate('Deposit', { currency });
        break;
      case 'scan':
  ...
  return (
    <View
      style={[
        ...
        variant === 'text' ? { paddingVertical: 8 } : { paddingHorizontal: 20 },
      ]}>
      {actions.map(item => (
        <WalletAction
          key={item.id}
