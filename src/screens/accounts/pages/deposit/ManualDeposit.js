...
import { ActivityIndicator,TouchableOpacity,  Pressable, TextInput } from 'react-native';
  ...
  return (
    <View f={1}>
      <SplitFlowLayout
        ...
        contentHeader={<DepositHeader  reference={wallet?.account} walletAccount={wallet?.currency?.code} />}
        contentDetail={<DepositDetail  account={bankAcc} walletAccount={wallet?.currency?.code} />}
        buttons={buttons}
      />
    </View>
  );
    ...
    <Pressable onPress={handleCopy}>
      {
        walletAccount == "USD" ? 
        <>
        <Text c="white" p={0.25} s={18} tA="center" fW="700" >Deposit via Plaid</Text>
        </>
        :
      <>
...
function DepositDetail(props) {
  ...
  const dwolla_plaid_integration_service='https://dwolla.krillpay.com/dwolla/fund-balance'
    ...
    <View>
      <View pt={1}>
        ...
        noTitle
            ...
            disabled={false}>
            <View f={1} pv={0.5} style={{marginTop:'5%', paddingTop:10, borderColor:'#f5f5f5', borderWidth:1, borderRadius: 10, padding:5}}>
        ...
        >
          ...
          style={{
           ...
           <TextInput
            ...
            >
             {!loading && <Text style={{color:'#fff', textAlign: 'center',}}>Proceed</Text> }
