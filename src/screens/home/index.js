...
export default function HomeScreen(props) {
	...
	const isBusinessGroup = checkBusinessGroup(businessServiceSettings, getUserGroup(user));

	return (
		<>
			<View screen hC={drawerOpen ? "primary" : "surface"} bC="#F8F8F8">
				<CurrentSessionsDrawer navigation={navigation} drawerOpen={drawerOpen} hideDrawer={() => setDrawerOpen(false)}>
					<React.Fragment>
						<AppMenuCompany setDrawerOpen={() => setDrawerOpen(true)} navigation={navigation} />
						<View f={1}>
				...
				{isBusinessGroup && business?.status === "verified" && (
					<View
						...
						<IconButton icon="pos" color="secondary" size={60} padding={16} onPress={() => navigation.navigate("PoS")} />
					</View>
				)}
			</View>
		</>
