  ...
  render() {
    ...
    return (
      <Animated.View style={containerStyle}>
        <LabelText style={[style, textStyle]} {...props}>
          {children}
        </LabelText>
      </Animated.View>
    );
...
function LabelText(props) {
  const { children, style, restProps, required } = props;
  const { t } = useTranslation(['common']);
  const i18nString = t(children);

  return (
    <Animated.Text style={style} {...restProps}>
      {i18nString ? i18nString : children}
      {required && <Text style={{ color: '#FF4C6F' }}> *</Text>}
    </Animated.Text>
  );
}
