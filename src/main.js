...
const Main = () => {
  ...
  if (isCompanyLoading || loading || !localAuth || !fontsLoaded) {
    return null;
  } else {
    return (
      <Providers {...{ localAuth, companyData }}>
        {companyNotFound ? (
          <View jC="center" aI="center" h="100%">
            <Image
              source={{
                uri: Image.resolveAssetSource(NotFound404).uri,
              }}
              style={{
                width: 200,
                height: 180,
...
const Providers = ({ children, localAuth, companyData }) => {
  return (
    <ToastProvider>
