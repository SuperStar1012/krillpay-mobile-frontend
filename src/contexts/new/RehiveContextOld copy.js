...
import authMachine, {
  ...
  COMPANY,
  LOGIN,
} from 'screens/auth_new/config/authMachine';
import {
  resetAuth,
  setTempCompany,
  removeAuthSession,
...
function RehiveProvider({ children, initial = {} }) {
  ...
  // const accounts = useAccounts({ init, services, config, tier, user });

  // useEffect(() => {
  //   if (accounts?.context?.error?.status === 401) {
  //     handleInvalidToken();
  //   }
  // }, [accounts?.context?.error]);
  ...
  const appLoading = !user || tier?.isLoading;
