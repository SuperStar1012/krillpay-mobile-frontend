...
export const krilltagSearch = searchTerm => {
  const value =
...
export function arrayToPlaceholder(array, options = {}) {
  ...
  return (
    ...
    placeholder.substr(1, 4) +
    krilltag +
    placeholder.slice(5).toLowerCase()
  );
}
...
export function getUserCountryFromMSISDN(msisdn) {
  let countryCode = null;
  if (msisdn.indexOf('+234') != -1) {
    countryCode = 'NG';
  } else if (msisdn.indexOf('+1') != -1) {
    countryCode = 'US';
  } else {
    //null
  }
